...
Imports append to independent sorter-style staging panels: sublists are collapsible, retain their source titles, and show which duplicate rows will be skipped by first-occurrence ID deduplication. The × controls soft-remove a source or item; marked rows stay visible and can be restored with ↺ before generation. The chart is not rendered until every timeline panel has staged items and **Generate chart** is clicked. Generation retains each complete deduplicated snapshot plus its removal markers, while only unmarked items appear in the chart. Completed slots that predate stored AniList label metadata are hydrated from cached media, character, staff, and studio records: every known media/name variant remains automatic regardless of the currently selected display language, while unmatched legacy labels are treated as custom. Studios have one canonical AniList name rather than language variants, but are still hydrated for custom-label detection and reset. Custom labels expose **Preserve custom labels** beside the import action and **Use AniList title** in the item editor; leaving preservation off uses the current AniList display-language preference.

The chart always matches exact logical IDs first. Gear → **Settings → Bump Chart → Best Match by Title** controls the persisted title fallback and is **on by default**. When enabled, unique exact stored labels and AniList title/name-language variants can join adjacent occurrences; exact logical IDs, unique labels, and unique alternate-title matches can also preserve a lineage across a gap. Stored custom labels remain eligible even when **Preserve custom labels** is off. Conflicting AniList identities never merge, and ambiguous duplicate titles stay disconnected. Every inferred adjacent connection carries a tooltip-enabled **info icon** near its arrival node. Changing a label so it no longer matches disconnects it; turning the setting off leaves exact logical-ID matching only.
