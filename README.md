...
Build an ordered ranking timeline from one or more **Previous order** panels and the final **Current order** panel. Click a panel's **Previous order N** or **Current order** heading to give that snapshot a custom name; press Enter or click away to save it, and clear the field to restore the positional default. Names stay attached to their snapshots when panels are promoted, moved, or removed. Use the translucent `+` beside Current order to append another Previous order, then use the arrow controls to swap adjacent snapshots—including Current order and the Previous order immediately before it. The final position remains the Current order slot while the staged snapshot and its custom name move together. On narrow layouts the add control moves below Current order. Every panel reuses the sorter's Single, Multiple/CSV, AniList, and completed-results importer, and reopening an importer returns to the tab most recently used in this Bump Chart workspace—even after a page reload. Staging areas scroll internally after about 560px while the Previous-order stack can keep growing down the page. **Clear staged**, immediately left of **Generate chart**, clears every panel and collapses timelines with extra panels back to the default Previous order/Current order pair.
