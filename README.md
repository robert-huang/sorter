...
Hover a path, node, or label row to emphasize that movement while fading the others. Matched paths show a centered movement badge while focused: rank gains are green and prefixed with `+`, rank losses are red, and unchanged ranks are gray. These interactive badges are intentionally omitted from PNG exports. Label links preserve the sorter interactions: supported AniList media/staff open detail modals on left-click, middle-click opens the source page, and right-click keeps the browser link menu. Clicking a rank opens the sorter item editor. **Export PNG** includes item images; AniList CDN covers use the same configured Cloudflare worker as the AniPlaylist search proxy because the CDN itself does not provide the CORS headers required by canvas. Use **Clear chart** to return to empty staging.
