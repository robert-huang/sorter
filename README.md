...
The active Bump Chart workspace autosaves every ordered column to IndexedDB after staging, editing, hiding, generating, clearing, reordering, changing import tabs, or changing **Best match by title**. Reloading always restores the active workspace to staging, including generated-chart items and hidden markers, so the chart can be reviewed before regenerating. Existing version-1 Before/After active and named saves load transparently as the default two-column version-2 timeline; all later writes use the ordered format. Hydration finishes before debounced autosave begins, so an empty initial render cannot replace stored work. **Clear chart** returns every complete snapshot to staging as one merged **From chart** group per panel; hidden entries remain marked and can be restored before regenerating. **Save chart…** creates a named generated-timeline snapshot. Named slots retain every column, its order, hidden markers, label preference, and view. Loading a named chart opens it directly as a generated chart and restores its matching/display settings; **Clear chart** returns that saved snapshot to staging.
