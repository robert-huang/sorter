...
Compare a pre-ranked **Before reorder** list with an **After reorder** list. Each side reuses the sorter's Single, Multiple/CSV, AniList, and completed-results importer, and reopening either importer returns to the tab most recently used in this Bump Chart session. Imports append to independent sorter-style staging panels: sublists are collapsible, retain their source titles, and show which duplicate rows will be skipped by first-occurrence ID deduplication. The × controls soft-remove a source or item; marked rows stay visible and can be restored with ↺ before generation. The chart is not rendered until both sides are staged and **Generate chart** is clicked. Generation snapshots the unmarked staged order and clears both importers. Completed slots that predate stored AniList label metadata are hydrated from cached media, character, staff, and studio records: every known media/name variant remains automatic regardless of the currently selected display language, while unmatched legacy labels are treated as custom. Studios have one canonical AniList name rather than language variants, but are still hydrated for custom-label detection and reset. Custom labels expose **Preserve custom labels** beside the import action and **Use AniList title** in the item editor; leaving preservation off uses the current AniList display-language preference.

The chart always matches exact logical IDs first. Gear → **Settings → Bump Chart → Best Match by Title** controls the persisted title fallback and is **on by default**. When enabled, the remaining rows are checked in order for a unique exact stored-label match and then a unique AniList title/name-language variant match. Stored custom labels remain eligible for matching even when **Preserve custom labels** is off and the chart displays the current AniList title instead. This bridges an archived pasted/manual result (whose auto-assigned ID differs) to a fresh AniList-backed result. A title match is accepted only when it identifies one item on each side; ambiguous duplicate titles stay disconnected, as do two conflicting AniList logical IDs. Every inferred connection carries an **info icon** near its right node, so changing either label to something that no longer matches disconnects it. Turning the setting off leaves exact logical-ID matching only.
...
Hover a path, node, or label row to emphasize that movement while fading the others. Click a path, node, or connected label to pin that lineage; the pinned line and both labels remain emphasized until another lineage is clicked or you click elsewhere. Matched paths show a centered movement badge while focused or pinned: rank gains are green and prefixed with `+`, rank losses are red, and unchanged ranks are gray. These interactive badges are intentionally omitted from PNG exports. Label links preserve the sorter interactions: supported AniList media/staff open detail modals on left-click, middle-click opens the source page, and right-click keeps the browser link menu. Clicking a rank opens the sorter item editor. **Export PNG** includes item images and inferred-match markers; AniList CDN covers use the same configured Cloudflare worker as the AniPlaylist search proxy because the CDN itself does not provide the CORS headers required by canvas. Use **Clear chart** to return to empty staging.
