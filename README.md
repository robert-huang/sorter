...
- **Show theme songs** — optional per-show theme song blocks under each cell, with Spotify playlist match indicators when a playlist is selected in gear → Spotify settings. Green is an exact Spotify id/ISRC match, red is a resolvable Spotify song absent from the playlist, and blue is a conservative title+artist metadata match to either a local file or an alternate Spotify catalog edition; unknown/unmatched source rows have no dot. Combined character/CV credits, parenthetical title translations, multilingual aliases, and reversed romanized name order can match when one complete performer name is identifiable. A distinctive exact title can also bridge artist tags written in non-overlapping scripts when there is no competing playlist identity; incompatible artists in a comparable script, ambiguous same-title files, short generic titles, and unrelated longer titles remain unmatched. Hover a blue dot to see the cached title, artists, playlist item number, and whether the match is local or catalog. Playlist refreshes cache title, artist, album, duration, and position metadata for catalog and local tracks (plus local URI), and the settings count includes both kinds. Click/right-click the playlist filter chip to show songs on / not on the playlist; blue metadata matches count as on-playlist.
